...
def evaluate_components(traces, event_kernel=5, robust_std=False):
    ...
    Such probability is defined as the likelihood of observing the actual trace value over N samples given an estimated
    ...
    :param traces: numpy.array of size NxM where N : number of neurons (rois), M: number of timestamps (Fluorescence traces)
    :param event_kernel: int, number of consecutive events
    :param robust_std: boolean indicating whether or not to use the interquartile range to calcluate standard deviation
    :return
        ...
        erfc: numpy.array; probability at each time step of observing the N consecutive actual trace values given the distribution of noise
              For each event, this is the sum of the log probabilities of finding a series of {event_kernel} events at least as extreme as the
              {event_kernel} sequence of events starting with designated event.
...
def get_trace_events(traces_y0, th_ag, len_ne):
    ...
    :param th_ag: scalar : threshold to find events on the trace; the higher the more strict on what we call an event.
                           This threshold is applied to the negative of the {erfc} array returned by evaluate_components
    ...
    [_, _, erfc] = evaluate_components(traces_y0, event_kernel=5, robust_std=False)
    erfc = -erfc
    ...
    for iu in range(traces_y0.shape[0]):
        ...
        if sum(evs[iu]) > 0:
            ...
            inds_final = inds_final[
                inds_final >= 0]  # to avoid the negative values that can happen due to taking 20 frames before an even
