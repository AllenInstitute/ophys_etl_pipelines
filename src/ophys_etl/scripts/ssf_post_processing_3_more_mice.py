...
"""Modified ssf_post_procesing script for the 3 non-"Golden" mice that have
a slightly different directory structure to the "Golden" mouse.
"""
...
def create_trace_input_json(output_dir: pathlib.Path,
                            ...
                            roi_dir: pathlib.Path) -> Tuple[pathlib.Path,
                                                            pathlib.Path]:
    """
    """
    # Setup output dir.
...
def create_demix_input_json(
        ...
        experiment_id: int,
        trace_output_json_path: pathlib.Path,
        motion_dir: pathlib.Path) -> Tuple[pathlib.Path,
                                           pathlib.Path]:
    """
    """
    # Setup output dir.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--motion_corrected_data_dir',
                        type=str,
                        help='Directory location of the motion correction.')
    parser.add_argument('--segmentation_data_dir',
                        type=str,
                        help='Directory location of the segmentations.')
    args = parser.parse_args()
    base_dir_path = pathlib.Path(args.output_path)
    motion_dir_path = pathlib.Path(args.motion_corrected_data_dir)
    ...
    trace_input_json_path, trace_output_json_path = create_trace_input_json(
        ...
        motion_dir=motion_dir_path,
        roi_dir=roi_dir_path)
    ...
    demix_input_json_path, demix_output_json_path = create_demix_input_json(
        output_dir=base_dir_path,
        experiment_id=args.experiment_id,
        trace_output_json_path=trace_output_json_path,
        motion_dir=motion_dir_path)
    ...
    dff_input_json_path, dff_output_json_path = create_dff_input_json(
        output_dir=base_dir_path,
        experiment_id=args.experiment_id,
        movie_frame_rate_hz=args.movie_frame_rate_hz,
        neuropil_output_json_path=npil_output_json_path)
    job = Popen(
