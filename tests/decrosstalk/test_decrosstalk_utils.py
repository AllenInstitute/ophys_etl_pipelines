...
@pytest.mark.parametrize("sig_indices,ct_indices,window,ind_indices",
                         [(np.array([0, 10, 11, 12, 21, 22,
                                     23, 24, 25, 30, 31]),
                           np.array([1, 9, 10, 25, 26, 27, 28, 32]),
                           1,
                           np.array([3, 4, 5, 6, 9])),
                          (np.array([1, 10, 11, 12, 21, 22,
                                     23, 24, 25, 30, 31]),
...
def test_find_independent_events(sig_indices,
    ...
    n_unq_events = len(np.unique(signal_events['trace']))
    assert n_unq_events == len(signal_events['trace'])

    crosstalk_events = {}
    crosstalk_events['events'] = ct_indices

    ind_events = du.find_independent_events(signal_events,
                                            crosstalk_events,
                                            window=window)
...
def test_validate_cell_crosstalk(sig_indices,
    ...
    (is_valid_roi,
        ind_events) = du.validate_cell_crosstalk(signal_events,
                                                 crosstalk_events,
                                                 window=window)
...
def test_validate_raw_traces():
    ...
    results = du.validate_traces(traces_dict)
