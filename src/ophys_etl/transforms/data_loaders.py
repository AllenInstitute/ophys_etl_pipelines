...
def get_max_correction_border(motion_correction_path: Path,
                              maximum_shift: float = 30.0) -> motion_border:
    """
    Gets the maximum correction values in the cardinal directions from a
    rigid motion transform file created during motion correction workflow
    step.
    Parameters
    ----------
    motion_correction_path: Path
        The path to the csv file that contains the motion correction data.
    maximum_shift: float
    ...
    Tuple[float, float, float, float]
        ...
        [Left, Right, Up, Down]
    """
    # load motion correction dataframe from file
    if not motion_correction_path.exists():
        raise FileNotFoundError("Motion Correction File Not Found")

    # pass to private function for calculation
    motion_correction_df = pd.read_csv(motion_correction_path)
    max_motion_border = _get_max_correction_values(motion_correction_df,
                                                   maximum_shift)
    return max_motion_border


def _get_max_correction_values(motion_corr_df: pd.DataFrame,
