...
from ophys_etl.modules.secular_classifier.classify import (
    find_best_circle)
...
def add_roi_to_img(
    ...
    if mn is not None:
        if mn is not None:
            img = np.where(img>mn, img, mn)
        if mx is not None:
            img = np.where(img<mx, img, mx)
        img = img - mn
        delta = mx-mn    
    else:
        img = img-img.min()
    ...
    img = np.round(255.0*img.astype(float)/delta).astype(np.uint8)
    ...
    return add_roi_contour_to_img(rgb, ophys, (255, 0, 0), 0.5)
