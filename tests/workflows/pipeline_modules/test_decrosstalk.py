...
class TestDecrosstalkModule(MockSQLiteDB):
        ...
        with Session(self._engine) as session:
            for oe_id in self._experiment_ids:
                save_job_run_to_db(
                    ...
                    module_outputs=[
                        OutputFile(
                            well_known_file_type=(
                                WellKnownFileTypeEnum.MOTION_CORRECTED_IMAGE_STACK # noqa E501
                            ),
                            path=(
                                self._tmp_dir / f"{oe_id}_motion_correction.h5"
                            ),
                        ),
                        OutputFile(
                            well_known_file_type=(
                                WellKnownFileTypeEnum.MAX_INTENSITY_PROJECTION_IMAGE # noqa E501
                            ),
                    ...
                    additional_steps=MotionCorrectionModule.save_metadata_to_db, # noqa E501
                )
                ...
                with open(self._tmp_dir / f"{oe_id}_max_proj.png", "w") as f:
                    f.write("")
                with open(
                    self._tmp_dir / f"{oe_id}_motion_correction.h5", "w"
                ) as f:
                    f.write("")
    ...
    def test_inputs(
        ...
        mock_ophys_experiment_from_id.side_effect = lambda id: OphysExperiment(
            id=id,
            ...
            imaging_plane_group=ImagingPlaneGroup(
                id=0 if id == 1 else 1, group_order=0 if id == 1 else 1
            ),
        ...
        with patch(
            "ophys_etl.workflows.pipeline_modules.decrosstalk.engine",
            new=self._engine,
        ):
            mod = DecrosstalkModule(
                docker_tag="main", ophys_session=mock_ophys_session
            )
            ...
            "coupled_planes": [
                {
                    ...
                    "group_order": (0 if self._experiment_ids[i] == 1 else 1),
                    "planes": [
                        {
                            "ophys_experiment_id": self._experiment_ids[i],
                            "motion_corrected_stack": str(
                                self._tmp_dir / f"{self._experiment_ids[i]}_"
                            ...
                            "output_roi_trace_file": str(
                                mod.output_path
                                / f"ophys_experiment_{self._experiment_ids[i]}_" # noqa E501
                                f"roi_traces.h5"
                            ),
    ...
    @patch.object(OphysExperiment, "from_id")
    def test_save_decrosstalk_flags_to_db(self, mock_ophys_experiment_from_id):
        ophys_session = OphysSession(id=1, specimen=Specimen(id="specimen_1"))
        ...
        mock_ophys_experiment_from_id.side_effect = lambda id: OphysExperiment(
            ...
            container=OphysContainer(id=1, specimen=ophys_session.specimen),
        ...
        with patch(
            ...
            save_job_run_to_db(
                ...
                module_outputs=[
                    OutputFile(
                        well_known_file_type=WellKnownFileTypeEnum.DECROSSTALK_FLAGS, # noqa E501
                        path=(
        ...
        expected_flags = {
            ...
            4: ["decrosstalk_invalid_raw", "decrosstalk_invalid_unmixed"],
        }
