...
import joblib
import numpy as np
from argschema import ArgSchemaParser, fields
from botocore.errorfactory import ClientError
from marshmallow import Schema, ValidationError, pre_load, validates
from marshmallow.validate import OneOf
from scipy.signal import resample_poly
from scipy.sparse import coo_matrix

from croissant.features import FeatureExtractor
from ophys_etl.types import SparseAndDenseROI
from ophys_etl.schemas import SparseAndDenseROISchema, ClassifierSchema
...
class InferenceInputSchema(ClassifierSchema):
    classifier_model_path = fields.Str(
        required=True,
        description=("s3 URI or local path to model. "
                     "The output of the model is 0 (not cell) or 1 (cell). "
                     "If not provided the model registry will be queried "))
    ...
    model_registry_env = fields.Str(
        ...
        validate=OneOf({'dev', 'stage', 'prod'},
                       error=("'{input}' provided for 'model_registry_env'. "
                              "Valid options are: {choices}.")),
        description=("Classifier model registry environment."))
