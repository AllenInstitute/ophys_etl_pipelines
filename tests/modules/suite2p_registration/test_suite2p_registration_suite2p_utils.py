...
try:
    from ophys_etl.modules.suite2p_registration.suite2p_utils import (  # noqa: E402, E501
        ...
        add_modify_required_parameters, create_ave_image, optimize_motion_parameters,
...
class TestRegistrationSuite2pUtils(unittest.TestCase):
    def setUp(self):
        ...
        self.h5_file_loc = self.resource_loc / "792757260_test_data.h5"
        self.mock_data_loc = self.resource_loc / "test_rand.npy"
    ...
    def test_load_initial_frames(self):
        ...
        n_frames_to_load = 20
        loaded_data = load_initial_frames(
            file_path=self.h5_file_loc,
            h5py_key="input_frames",
            n_frames=n_frames_to_load,
        )
        expected_frames = np.linspace(
            0, self.n_frames, n_frames_to_load + 1, dtype=int
        )[:-1]
        self.assertTrue(
            np.all(np.equal(loaded_data, self.frames[expected_frames, :, :]))
        )
    ...
    def test_compute_reference_with_mocks(self):
        ...
        mock_frames = self.rng.integers(
            -self.n_frames,
            self.n_frames,
            size=(self.n_frames, self.xy_shape, self.xy_shape),
        )
        ...
        with patch(
            "ophys_etl.modules.suite2p_registration.suite2p_utils."
    ...
    def test_remove_extrema_frames(self):
        ...
        frame_value = 100
        frame_scatter = 10
        test_data = self.rng.integers(
            low=frame_value - frame_scatter,
    ...
    def test_compute_reference(self):
        ...
        self.assertTrue(
            np.all(np.equal(self.original_reference, result_reference))
        )
    ...
    def test_optimize_motion_parameters(self):
        ...
        def create_ave_image_mock(
            ...
            if (
                suite2p_args["smooth_sigma"] > 0.65
                and suite2p_args["smooth_sigma_time"] > 0.0
            ):
                image[:50, :] = 10
                return {
                    "ave_image": image,
    ...
    def test_create_ave_image_mock(self):
        ...
        mock_frames = np.ones(
            (self.n_frames, self.xy_shape, self.xy_shape), dtype=int
        )
        max_shfit = 10

        def register_frames_mock(refAndMasks, frames, ops):
            y_shift = self.rng.integers(
                -max_shfit, max_shfit, size=self.n_frames
            )
            x_shift = self.rng.integers(
                -max_shfit, max_shfit, size=self.n_frames
            )
            return (
                mock_frames,
        ...
        self.assertTrue(
            np.allclose(
                result["ave_image"], np.ones((self.xy_shape, self.xy_shape))
            )
        )
    ...
    def test_create_ave_image(self):
        ...
        add_modify_required_parameters(suite2p_args)
        mock_frames = load_representative_sub_frames(
            suite2p_args["h5py"], suite2p_args["h5py_key"]
        )
        result = create_ave_image(
            ref_image=self.original_reference,
            input_frames=mock_frames,
